...
public class Battle{//Temporary
    ...
    public static void battler(Player player, Entity enemy){
        ...
        String skillClass = "mage", loseText = "defeated" + enemy; //Temporary
        boolean skip, run;
        int win = 0;
        abilityUses = 1;
        while(win == 0){
            ...
            if(skip) continue;
            if(win==1){
                System.out.printf("You defeated %s! They dropped %d coins.",enemy, 2);
                coins += 2;
            }else if(win==-1){
                System.out.printf("You %s and dropped %d coins!",loseText, 2);
                coins -= 2;
            }
        }
    }
    ...
    public static void battleCalculator(Entity attacker,Entity defender){
        ...
        if(Math.random() <= attacker.getDodgeChance()) System.out.println("Attack dodged!");
            ...
            if(damage <= defender.getDef()){
                ...
                System.out.printf("%s dealt %.0f HP to %s!",attacker,(damage - defender.getDef()),defender);
            }
        }
